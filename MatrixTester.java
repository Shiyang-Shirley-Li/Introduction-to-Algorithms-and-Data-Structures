...
public class MatrixTester {
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("Success: constructing a matrix from an empty array");
			caughtRightException = true;
		}
		if (!caughtRightException) {
			System.out.print("Fail: constructing a matrix from an empty array");
		...
		if (!m1Expected.equals(m1)) {
			System.out.println("Equals error: m1 and m1Expected not equal\nGot:" + m1 + "\nexpected:" + m1Expected + ")");
		}
		...
		Matrix mNull = null;
		if (m1.equals(mNull)) {
			System.out.println("Equals error: m1 and mNull equal, but it should not be equal");
		}
		...
		int mInt = 0;
		if (m1.equals(mInt)) {
			System.out.println("Equals error: m1 and mInt equal, but it should not be equal");
		}
		...
		try {
			...
			System.out.println("Success: we cannot do times on incompatible matrices");
			caughtRightException = true;
		}
		if (!caughtRightException) {
			System.out.print("Fail: we are not supposed to do times on incompatible matrices");
			caughtRightException = false; // reset for next such test case
		}
		...
		try {
			...
			System.out.println("Success: we cannot do plus on matrices of different dimensions");
			caughtRightException = true;
		}
		if (!caughtRightException) {
			System.out.print("Fail: we are not supposed to do plus on matrices of different dimensions");
			caughtRightException = false; // reset for next such test case
		}
	}
}
